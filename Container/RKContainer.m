...
- (void)registService:(Class)serviceClass forProtocol:(Protocol *)protocol{
    ...
    if ([RKUtils validateString:serviceImplStr]) {
        if (self.context.env == RKEnvTypeDebug) {
            @throw [NSException exceptionWithName:NSInternalInconsistencyException
    ...
    if ([RKUtils validateString:protocolName] && [RKUtils validateString:protocolService]) {
        [self.lock lock];
...
- (void)loadAllModuleAndService{
    // 读取所有modules
    NSMutableArray *modules = [RKConfigReader readConfigFromSectionName:RKModuleSectionName];
    NSLog(@"%@", modules);
    ...
    // 读取所有services
    NSMutableArray *servicesInfoArray = [RKConfigReader readConfigFromSectionName:RKServiceSectionName];
