...
static const char * kRKUIButtonLayoutStyleKey;
static const char * kRKUIButtonLayoutSpacingKey;
...
- (void)layoutButtoFrame
{
     ...
     */
    if (self.layoutStyle == RKImageLeftTitleRightStyle) {
    ...
    else if (self.layoutStyle == RKTitleLeftImageRightStyle) {
        ...
        if ([@(height) compare:@(currentHeigth)] != NSOrderedSame
            ...
            if (self.layoutStyle == RKImageTopTitleBottomStyle){
                /**
                 *  image往上移动
                 */
            ...
            }else if (self.layoutStyle == RKTitleTopImageBottomStyle){
                /**
                 *  title往上移动
                 */
...
- (void)setLayoutStyle:(RKUIButtonLayoutStyle)layoutStyle
{
    /**
     *  恢复默认状态
     */
    ...
    objc_setAssociatedObject(self, &kRKUIButtonLayoutStyleKey, @(layoutStyle), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    self.needLayoutForChangeStyle = YES;
}

- (RKUIButtonLayoutStyle)layoutStyle
{
    NSNumber *layoutStyleNum = objc_getAssociatedObject(self, &kRKUIButtonLayoutStyleKey);
    return layoutStyleNum.integerValue;
}

- (void)setLayoutSpacing:(CGFloat)layoutSpacing
{
    objc_setAssociatedObject(self, &kRKUIButtonLayoutSpacingKey, @(layoutSpacing), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    self.needLayoutForChangeStyle = YES;
}

- (CGFloat )layoutSpacing
{
    NSNumber *spacingNum = objc_getAssociatedObject(self, &kRKUIButtonLayoutSpacingKey);
...
- (void)expadnLayoutSubViews
{
    ...
    if (self.layoutStyle != RKDefalutStyle) {
