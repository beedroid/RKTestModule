...
NSString *const OTS_KEYCHAIN_DEVICECODE = @"OTS_KEYCHAIN_DEVICECODE";
...
{
    NSString *code=nil;
    NSString *keyChain_deviceCode = [RKKeychain getKeychainValueForType:OTS_KEYCHAIN_DEVICECODE];
    if (keyChain_deviceCode == nil || keyChain_deviceCode.length<10) {  // 若keychain里面没有或者长度因为什么原因很短的话，重新获取一次再储存
        ...
        [RKKeychain  setKeychainValue:code forType:OTS_KEYCHAIN_DEVICECODE];
    } else {
